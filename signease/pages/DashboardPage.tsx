...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fonction optimisée pour mettre à jour les documents unifiés
    const updateUnifiedDocuments = async () => {
      try {
        ...
        // ✅ BATCH: charger toutes les enveloppes en 1 requête au lieu de N requêtes individuelles
        const docsToLoad = sentDocs.filter((doc) => !deletedItems.has(doc.id));
        const envelopesMap = await getEnvelopesByDocumentIds(
          docsToLoad.map((doc) => doc.id)
        );

        const unifiedSentDocs: UnifiedDocument[] = docsToLoad.map((doc) => ({
          ...doc,
          source: "sent" as const,
          recipients: envelopesMap.get(doc.id)?.recipients || [],
        }));

        // Convertir les emails reçus en UnifiedDocument (sans requête réseau supplémentaire)
        const unifiedReceivedDocs: UnifiedDocument[] = receivedEmails
          .filter((email) => {
            const emailItemId = `email-${email.id}`;
            return !deletedItems.has(emailItemId);
          })
    ...
    // Debounce pour éviter la double exécution quand les 2 listeners arrivent quasi simultanément
    const debouncedUpdate = () => {
      if (debounceTimer) clearTimeout(debounceTimer);
      debounceTimer = setTimeout(updateUnifiedDocuments, 100);
    };
